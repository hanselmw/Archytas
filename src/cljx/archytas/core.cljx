(ns archytas.core
  (:require [archytas.messages :as m]
            ...
            [archytas.stream.spi :as spi :refer [read!]]
            [archytas.sysex :refer [read-sysex-event]]
            [archytas.util :as util :refer [lsb msb]]
...
(defn- open-board-chan
  ...
  (let [board-state (atom {:digital-out (zipmap (range 0 MAX-PORTS) (take MAX-PORTS (repeat 0)))
    ...
    (go 
      ...
      (spi/listen port (archytas-handler {:state board-state 
                                         :channel read-ch 
...
(defn open-board
  "Opens a connection to a board over a given ArchStream.
